"""Rede DDQN para detecção de anomalias, série No jump."""
...
print(f'art_daily_nojump.csv : {temporal_serie.shape}')
...
for i in tqdm(range(n_iteracoes)):
    ...
    random_indices = random.sample(
        range(window_size, len(temporal_serie)-1), 1000)

    for idx in tqdm(random_indices):
        state = temporal_serie\
            .loc[idx-window_size+1:idx, ['cond_std', 'dy_dt', 'value']]\
            .values.flatten().reshape(1, -1)
        next_state = temporal_serie\
            .loc[idx-window_size+2:idx+1, ['cond_std', 'dy_dt', 'value']]\
            .values.flatten().reshape(1, -1)

        action = select_action(state, exploration_rate)

        if (action == 1 and temporal_serie['anomaly'].iloc[idx] == 1):
            reward = 10
        elif (action == 1 and temporal_serie['anomaly'].iloc[idx] == 0):
            reward = -2
        elif (action == 0 and temporal_serie['anomaly'].iloc[idx] == 1):
            reward = -10
        elif (action == 0 and temporal_serie['anomaly'].iloc[idx] == 0):
            reward = 0.3

        if (len(memory) > replay_memory_size):
            # print(cao_escolhida_history.shape)
            del memory[0]
...
predictions, valores_df_prediciton, anomalias_previstas = (
    predict_full_series(temporal_serie, window_size))
real_anomaly = temporal_serie['anomaly'].loc[window_size:len(temporal_serie)]
df_real_linha = temporal_serie['value'].loc[window_size:len(temporal_serie)]
...
dnq_f1 = f1_score(real_anomaly, predictions)
print(f'Rede F1 Score : {dnq_f1}')

media = np.mean(predictions)
desvio_padrao = np.std(predictions)
minimo = np.min(predictions)
maximo = np.max(predictions)
quartis = np.percentile(predictions, [25, 50, 75])
soma = np.sum(predictions)
...
for idx in range(window_size, len(temporal_serie)):
    if temporal_serie['anomaly'].iloc[idx] == 1:
        # Adiciona o índice se a ação for igual a 1
        indices_anomolos_corretos.append(idx)
anomalias_reais_tupla = from_list_points_timestamps(indices_anomolos_corretos)

# F1 score MIT
data = pd.DataFrame({'timestamp': range(window_size, len(temporal_serie))})
start = window_size
end = len(temporal_serie) - 1
